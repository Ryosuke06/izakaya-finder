...
- コマンド実行前には、読み取り専用コマンドを含めて、目的・実行予定コマンド・影響範囲をユーザーに提示し、明示的な許可を得る。
- 例外は、ユーザーがそのコマンド実行を明示的に依頼した場合のみとする。
- ファイル編集、依存関係追加、サーバー起動、テスト実行、ネットワークアクセスを伴う可能性があるコマンドは、特に事前確認を必須とする。
